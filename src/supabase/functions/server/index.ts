...
app.post('/make-server-beba9171/api/candidates', async (c) => {
  try {
    ...
    return c.json({ success: true, key, accessCode });
  } catch (error) {
...
app.get('/make-server-beba9171/api/candidates', async (c) => {
  try {
    ...
    const validEntries = candidateEntries.filter(entry => 
    ...
    return c.json({ error: 'Failed to fetch candidates', details: String(error) }, 500);
  }
});
...
app.post('/make-server-beba9171/api/questionnaire/:accessCode/submit', async (c) => {
  try {
    ...
    const status = evaluation.score >= 18 ? 'shortlisted' : 'rejected';
